...
class Meccano_video(torch.utils.data.Dataset):
    """
    ...
    def __getitem__(self, index):
        ...
        video_path = os.path.join(
            '/home/GC/babdulrahman/rgbs/content'+self.mode, f"{folder_name}_{name_frame}.mp4"
        )
        depth_path = os.path.join(
            '/home/GC/babdulrahman/rgbs/content'+self.mode+'d', f"{folder_name}_{name_frame}.mp4"
        )
        ...
        if os.path.exists(video_path):
            ...
            while(frame_count <= int(self._frame_end[index][:-4])):
                ...
                if(len(name_frame) == 4): #add a prefix 0
                    ...
                    name_frame = "0000"+name_frame
                image = Image.open(self.cfg.DATA.PATH_TO_DATA_DIR+self.mode+"/"+self._path_to_videos[index]+"/"+name_frame+".jpg")
                image = np.asarray(image)
                frames.append(torch.from_numpy(image.copy()))
                frame_count+=1
            frames= torch.stack(frames)
        #sampling frames
